        ...
        return middleware.map(middlewareItem => {
            ...
            if (m instanceof BaseMiddleware) {
                ...
                return (req: express.Request, res: express.Response, next: express.NextFunction) => {
                    ...
                    //assim permitimos que cada requisição possua um instancia individual do middleware e estes possuam a arvore de dependencia correta
                    const httpContext: interfaces.HttpContext = this._getHttpContext(req);
                    const middlewareForRequest = httpContext.container.get(middlewareItem);
                    Reflect.set(middlewareForRequest, "httpContext", httpContext);

                    middlewareForRequest.handler(req, res, next);
                };
            }
